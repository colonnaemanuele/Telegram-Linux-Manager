...
def get_main_menu():
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("💽 Disk Usage", callback_data="cmd_disk_prompt"),
...
def get_scripts_menu(files):
    ...
    for file_name in files:
        keyboard.append([InlineKeyboardButton(f"🚀 {file_name}", callback_data=f"run_{file_name}")])
...
def get_disk_retry_root_menu(path):
    """Menu mostrato se il controllo disco fallisce o è parziale"""
    keyboard = [
        [InlineKeyboardButton("🔥 Riprova come ROOT", callback_data=f"retry_disk_root|{path}")],
